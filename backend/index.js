...
createTables();
const Frontend_url = "https://job-board-seven-ashen.vercel.app";
//  ||
app.use(
  cors({
...
app.get("/downloadResume/:applicationId", async (req, res) => {
  ...
  try {
    const application = await applicationModel.findApplicationById(
      applicationId
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename="${application.resume_filename}"`
    );
...
app.post("/getCandidateApplications", async (req, res) => {
  const userId = req.body.userId;
  try {
    const applications = await applicationModel.findApplicationsByCandidateId(
      userId
    );
    const applicationsWithUrls = addResumeUrls(applications, req);
